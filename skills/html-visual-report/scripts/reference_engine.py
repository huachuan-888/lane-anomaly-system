...
def dual_check(csv_labels, frame, cfg):
    ...
    if not det["left_ok"] or not det["right_ok"]:
        miss_side = "右" if det["left_ok"] else "左"
        hit_side = "左" if det["left_ok"] else "右"
        if csv_type == "丢失":
            return "✅ 真实丢失", f"CSV报丢失, 画面仅{hit_side}侧有线({miss_side}侧丢失), 画面侧丢失属实"
        return "🔍 需人工复核", f"画面仅检测到{hit_side}侧车道线, {miss_side}侧疑似丢失, 建议人工确认"
...
def generate_html(results, total_points, total_frames, cfg, out_time):
    ...
    for e in results:
        ...
        for idx, pt in enumerate(points[: cfg["max_points"]], 1):
            # 先做画面检测: 单侧丢失时把"画面X侧丢失"加入标签 (像抖动/过宽一样显示)
            ...
            if b1_fidx is not None and b1_fidx in video_frames:
                ...
                if _det["left_ok"] and _det["right_ok"]:
                    ...
                    # 仅检测到单侧 = 另一侧车道线丢失
                    miss_side = "右" if _det["left_ok"] else "左"
                    hit_side = "左" if _det["left_ok"] else "右"
                    hit_x = _det["left_x"] if _det["left_ok"] else _det["right_x"]
                    _det_txt = f" | 画面检测: 仅检测到{hit_side}侧车道线({hit_x}px), {miss_side}侧丢失"
                    # 把画面侧丢失加入校验标签 (画面证据)
                    if "丢失" not in " ".join(pt["labels"]):
                        pt["labels"] = list(pt["labels"]) + [f"画面{miss_side}侧丢失"]
                else:
...
def main():
    ...
    parser.add_argument("--out", default=None, help="自定义输出HTML路径")
    ...
    run_engine(args.base, args.out, args.window, args.max_points)


def run_engine(base_dir, out_html=None, window=None, max_points=None):
    """完整分析引擎: 问题表 -> CSV检测 -> 视频抽帧 -> 校验 -> 报告
    返回 (html_path, total_points, total_frames)
    """
    cfg = dict(CONFIG)
    cfg["base_dir"] = base_dir
    if window:
        cfg["window_csv"] = window
    if max_points:
    ...
    if out_html:
        # 自定义输出路径 (技能模式)
        html_path = out_html
        md_path = os.path.join(os.path.dirname(out_html), os.path.splitext(os.path.basename(out_html))[0] + ".md")
    ...
    return html_path, total_points, total_frames
